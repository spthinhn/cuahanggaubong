<?php
	...
	include_once "../phpMailer/class.phpmailer.php";	
		...
		$body .= '
				<tr>
					...
					<td align="right";><img src="http://www.'.$config_url.'/'._upload_hinhanh_l.$logo_top['thumb_'.$lang].'" ></td>
					</tr>
					</table>
		...
		<table border="1" bordercolor="#0099CC" align="center" cellpadding="5px" cellspacing="5px" width="100%">';
			if(is_array($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++){
					...
					$gia=get_fc_size_price($check)['price'];
                    ...
                    $body.='<td>'.number_format($gia,0, ',', '.').'&nbsp;VND</td>';
                    $body.='<td>'.$q.'</td>';                 
                    $body.='<td>'.number_format($gia*$q,0, ',', '.') .'&nbsp;VND</td>
                    </tr>';
				}
  ...
  <tr>
			...
			$sql = "INSERT INTO  table_order (madonhang,hoten,dienthoai,diachi,email,httt,tonggia,noidung,ngaytao,trangthai,tinhthanh,quanhuyen,phivanchuyen)  VALUES ('$mahoadon','$ten_input','$dienthoai_input','$diachi_input','$email_input','$phuongthuc_input','$tonggia','$noidung_input','$ngaydangky','1','$tinhthanh','$quanhuyen','$phivanchuyen')";
			mysql_query($sql);
			...
			if($mail->Send()){				
				for($i=0;$i<$max;$i++){
					$pid = $_SESSION['cart'][$i]['productid'];
					$q = $_SESSION['cart'][$i]['qty'];
					$soluong = "UPDATE #_product SET soluong=soluong-$q  WHERE id ='".$pid."'";
					$d->query($soluong);
				}
				echo 1;
