<?php  if(!defined('_source')) die("Error");
		...
		if($id!=''){
			...
			$fc_sql = "select * from #_size where product_id = ".$row_detail['id']." order by size";
			$fc_stmt = $d->query($fc_sql);
			$html = '';
			$fc_price = 0;
			$i = 0;
			while ($fc_row=@mysql_fetch_array($fc_stmt)) 
			{
				if ($i == 0) {
					$fc_price = $fc_row['price'];
				}
				$html .= '<option price="'.number_format($fc_row['price'],0,",",".")." ₫" .'" value="'.$fc_row['id'].'">'.$fc_row['size'].'</option>';
				$i++;
			}
			...
			foreach ($product_banchay as $key => $value) {
				$fc_sql = "select * from #_size where product_id = ".$product_banchay[$key]['id']." order by size limit 1";
				$fc_stmt = $d->query($fc_sql);
				$test = $d->fetch_array();
				$product_banchay[$key]['price'] = $test['price'];
			}
			...
			$d->reset();
			$sql = "select id,thumb,ten_$lang,giaban,tenkhongdau,giacu,rate,mota_$lang from #_product where hienthi=1 and type='product' and type='product' and id_list='".$row_detail['id_list']."' and id!='".$row_detail['id']."' order by stt,ngaytao desc limit 10";
			$d->query($sql);
