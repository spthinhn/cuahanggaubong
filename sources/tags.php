...
#các sản phẩm khác======================
	...
	if($idl!=''){
		...
		$sql = "select id,ten_$lang,thumb,tenkhongdau,giaban,giacu,mota_$lang from $where $limit";
		$d->query($sql);
		...
		if(count($product)==0){ 
			$where = " #_baiviet where hienthi=1 and FIND_IN_SET(".$row_detail['id'].",tags) ".$sql_sort;
