...
{
	$sql = "select * from table_size where product_id =".$_GET['id'];
	$stmt=mysql_query($sql);
	$html = '';
	while ($row=@mysql_fetch_array($stmt)) 
	{
		$html .= '<tr><td><input type="text" name="fc_size[]" title="Nhập kích thước" class="tipS validate[required]" value="'.$row['size'].'" /></td><td><input type="text" name="fc_giaban[]" title="Nhập giá bán" class="conso tipS validate[required]" value="'.$row['price'].'" /></td><td><a class="fc_size_delete" href="javascript:void(0)"><img class="fc_size_img" src="images/fc_delete.png" /></a></td></tr>';
	}
	return $html;
}
  //------------ tags-------------------------
...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
    ...
    <?php }?>
		...
		<div class="formRow" style="border-bottom: 0px;">
			<div id="fc_size" class="formRight">
					<table id="fc_table_size" style="width: 100%">
						<tr>
							<th width="40%">
						...
						<?php } else { ?> 
							<tr>
								<td>
									<input type="text" name="fc_size[]" title="Nhập kích thước" class="tipS validate[required]" value="" />
								</td>
								<td>
									<input type="text" name="fc_giaban[]" title="Nhập giá bán" class="conso tipS validate[required]" value="" />
								</td>
								<td>
									<a class="fc_size_delete" href="javascript:void(0)"><img class="fc_size_img" src="images/fc_delete.png" /></a>
								</td>
							</tr>
						<?php } ?>
					</table>
		<a style="float:right;" href="javascript:void(0)" id="fc_add_size"><img src="images/fc_add.png" /></a>
			</div>
		</div>
		...
		<div class="formRow">
			...
			<div class="formRight">
                <input type="text" name="soluong" title="Nhập số lượng" id="soluong" class="tipS validate[required]" value="<?=@$item['soluong']?>" />
			</div>
			<div class="clear"></div>
		</div>
		...
		<div class="formRow">
			...
			<div class="formRight">
                <input type="text" name="link" title="Nhập Link youtube" id="link" class="tipS" value="<?=@$item['link']?>" />
			</div>
			<div class="clear"></div>
		</div>
...
<script>
  $(document).ready(function() {
    ...
    $(document).on('click', '#fc_add_size', function(){
    	var str = '<tr><td><input type="text" name="fc_size[]" title="Nhập kích thước" class="tipS validate[required]" value="" /></td><td><input type="text" name="fc_giaban[]" title="Nhập giá bán" class="conso tipS validate[required]" value="" /></td><td><a class="fc_size_delete" href="javascript:void(0)"><img class="fc_size_img" src="images/fc_delete.png" /></a></td></tr>';
    	
    	$('#fc_table_size tr:last').after(str);
    });
    $(document).on('click', '.fc_size_delete', function(){
