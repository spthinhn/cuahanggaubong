...
function save_man(){
	...
	if($id){
		$id =  themdau($_POST['id']);
		$fc_sql = "select * from #_size where product_id = ".$id;
		$fc1->query($fc_sql);
		if($fc1->num_rows()>0){
			$fc_sql = "delete from #_size where product_id = ".$id;
			$fc2->query($fc_sql);
		}
		foreach ($_POST['fc_size'] as $keySize => $valSize) {
			$data2['size'] = $valSize;
			$data2['price'] = str_replace(",","",$_POST['fc_giaban'][$keySize]);
			$data2['product_id'] = $id;
			$fc3->setTable('size');
			$fc3->insert($data2);
		}
		...
		$data['ngaytao'] = time();
		$data['size'] = 0;
		// $data['size2'] = $_POST['size2'];
		// $data['giaban2'] = $_POST['giaban2'];
		$d->setTable('product');
		if($d->insert($data))
		{	
